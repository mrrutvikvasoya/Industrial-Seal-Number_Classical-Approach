"""Find the seven digit boxes of a seal code without recognizing them."""
from dataclasses import dataclass

import cv2
import numpy as np

from seals.config import DIGIT_COUNT, MAX_ANGLE
from seals.rows import HEIGHT_RATIO_LIMITS, aligned_rows
from seals.thresholds import threshold_masks
...
MIN_FILL = 0.20
# Reject irregular rows that admit background; this score is not a probability.
MAX_ROW_SCORE = 0.20


@dataclass(frozen=True)
class Localization:
    """Seven ordered (x, y, w, h) boxes plus the row's polarity, tilt, score, and method."""
    digit_boxes: tuple
    polarity: str
    angle: float
    score: float
...
def digit_components(mask):
    """Keep connected components whose height, aspect, and fill look like a digit."""
    _, _, statistics, centers = cv2.connectedComponentsWithStats(mask, connectivity=8) # finds every connected white blob and gives its bounding box
    components = []
    for index, (x, y, width, height, area) in enumerate(statistics):
        if index == 0:
            continue # index 0 is the background, skip it
...
def row_geometry(row):
    """Score a row's regularity (lower is better) and estimate its tilt in degrees."""
    ...
    if height_ratios.min() < HEIGHT_RATIO_LIMITS[0] or height_ratios.max() > HEIGHT_RATIO_LIMITS[1]:
        return float('inf'), 0.0 # heights too uneven → reject
    centers = np.array([box[4:6] for box in row])
    gaps = np.diff(centers[:, 0]) # horizontal gaps between digits
    if np.any(gaps <= 0):
        return float('inf'), 0.0 # overlapping/out of order → reject
...
def candidate_rows(components):
    """Group components into horizontal bands and slice each into digit-count windows."""
    if len(components) < DIGIT_COUNT:
        return [] # fewer than 7 blobs → impossible
    ordered = sorted(components, key=lambda box: box[5]) # sort by `center`-y (top→bottom)
    ...
    rows = []
    for group in groups:
        horizontal = sorted(group, key=lambda box: box[4]) # sort each band left→right
...
def restore_boxes(row, source_shape, resized_shape):
    """Map half-resolution boxes back to full-image pixels, rounding outward within bounds."""
...
def find_threshold_row(components_by_polarity, method, source_shape, resized_shape, use_alignment=False):
    """Pick the lowest-scoring acceptable row across polarities for one threshold method."""
    best = None
...
def find_digits(gray):
    """Return a Localization for the seven-digit row, or None if no row is found."""
    ...
    for method in ('otsu', 'adaptive'):
        ...
        if baseline is not None:
            return baseline
    # The adaptive components are already computed; reuse them for the aligned retry.
