"""Group components that lie on one tilted line into candidate digit rows."""
...
# Matches the localizer's supported tilt range.
MAX_SLOPE = float(np.tan(np.radians(MAX_ANGLE)))


def aligned_rows(components, digit_count=DIGIT_COUNT):
    """Return every run of `digit_count` similar-height components along a shared line."""
    rows = {}
    # Try every pair of blobs — each pair defines one candidate ruler (line direction).
    for first, second in combinations(components, 2):
        # If the pair is too close horizontally
        horizontal_distance = second[4] - first[4]
        mean_height = (first[3] + second[3]) / 2
        if abs(horizontal_distance) < mean_height:
            continue
        # Compute the ruler's tilt (the green line's slope). If steeper than 15°, it can't be a digit row
        slope = (second[5] - first[5]) / horizontal_distance
        if abs(slope) > MAX_SLOPE:
            continue
        aligned = []
        # Test each blob against two criteria: height ratio and line residual
